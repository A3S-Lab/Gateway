...
| --- | --- | --- |
| HTTP, SSE, WebSocket, gRPC, TCP, UDP, TLS, bounded graceful drain, routing, load balancing, health, and atomic reload | Available; HTTP-derived paths remove fixed and `Connection`-nominated hop-by-hop fields at each proxy boundary; ordinary HTTP request bodies stream but ordinary responses are still collected before downstream delivery, and the `compress` middleware quality-negotiates eligible buffered responses while preserving streaming protocol bodies; gRPC relays request and response DATA frames plus trailers over h2c, uses one bounded streaming path for every upstream response, buffers only a selected mirror request for replay, normalizes `TE: trailers`, regenerates normalized `X-Forwarded-*` metadata, and applies per-service first-response, idle-stream, and total-operation bounds; WebSocket validates the HTTP/1.1 opening handshake, completes a service-timeout-bounded upstream handshake before returning `101`, preserves non-`101` status and safe end-to-end headers with a Gateway-generated JSON body, forwards end-to-end request headers and negotiated subprotocols, and relays application messages without Gateway-defined control prefixes; SSE has the same independent stream bounds; shutdown closes listeners before drain, tracks long-lived work, force-cancels at the configured deadline, and does not report `Stopped` before task cleanup | Replace ordinary HTTP response collection with a bounded downstream stream before claiming full-duplex HTTP; continuously regress gRPC/trailer and bidirectional header boundaries, transparent WebSocket relay, handshake rejection/failure/timeout cleanup, and the pinned official OpenAI Python SDK four-endpoint gate |
...
18. **Complete (2026-08-03):** replace the marker-only `compress` middleware
    ...
    response streaming remains separate open work.
