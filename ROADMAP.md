...
| Performance evidence | Criterion regression baselines publish internal route, middleware, and ACL parse costs. A same-host HTTP/1.1 comparison publishes five alternating A3S Gateway/NGINX trials with one shared upstream: in the current small-response baseline, NGINX records higher throughput and lower P50/P90/P99 latency (28,812 versus 62,091 req/s; 4.68 versus 3.39 ms P99) | Treat the result as a concrete HTTP hot-path baseline. Preserve raw trials, environment, versions, limits, and the narrow workload label; add separate TLS and streaming profiles before making broader claims |
