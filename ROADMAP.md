...
| --- | --- | --- |
| HTTP, SSE, WebSocket, gRPC, TCP, UDP, TLS, bounded graceful drain, routing, load balancing, health, and atomic reload | Available; HTTP-derived paths remove fixed and `Connection`-nominated hop-by-hop fields at each proxy boundary; ordinary HTTP requests and responses use one full-duplex Hyper relay to HTTP or certificate-verified HTTPS upstreams, negotiate HTTP/1.1 or HTTP/2 through TLS ALPN, preserve downstream backpressure and safe trailers, apply per-service first-response/idle-body/total-operation bounds, and retain body-lifetime backend, admission, telemetry, access-log, and usage accounting; the `compress` middleware quality-negotiates eligible responses through at most 8 MiB of look-ahead and replays an overflowing prefix without loss; gRPC relays request and response DATA frames plus trailers over h2c, uses one bounded streaming path for every upstream response, buffers only a selected mirror request for replay, normalizes `TE: trailers`, regenerates normalized `X-Forwarded-*` metadata, and applies the same independent bounds; WebSocket validates the HTTP/1.1 opening handshake, completes a service-timeout-bounded upstream handshake before returning `101`, preserves non-`101` status and safe end-to-end headers with a Gateway-generated JSON body, forwards end-to-end request headers and negotiated subprotocols, and relays application messages without Gateway-defined control prefixes; SSE has the same independent stream bounds; shutdown closes listeners before drain, tracks long-lived work, force-cancels at the configured deadline, and does not report `Stopped` before task cleanup | Preserve full-duplex HTTP/gRPC overlap, verified upstream TLS, trailer and bidirectional header boundaries, transparent WebSocket relay, handshake rejection/failure/timeout cleanup, and the pinned official OpenAI Python SDK four-endpoint gate |
| Middleware extension boundary | Available for embedded Rust deployments: `MiddlewareRegistry` binds typed `Middleware` values to stable router-facing names, validation rejects missing references and ACL name collisions, compiled pipelines preserve request order and reverse response order, and the immutable registry survives atomic configuration reloads | Keep the boundary typed and compile-time. Do not claim standalone dynamic-library or Wasm plugin loading without a versioned ABI, isolation, resource bounds, and recovery evidence |
| Performance evidence | Criterion regression baselines publish internal route, middleware, and ACL parse costs. A same-host HTTP/1.1 comparison publishes five alternating A3S Gateway/NGINX trials with one shared upstream: the current small-response baseline rates Gateway worse on throughput and P50/P90/P99 latency (28,812 versus 62,091 req/s; 4.68 versus 3.39 ms P99) | Treat the result as a concrete HTTP hot-path optimization target. Preserve raw trials, environment, versions, limits, and the narrow workload label; add separate TLS and streaming profiles before making broader claims |
...
12. **Complete (2026-08-03):** validate downstream WebSocket upgrade method,
    ...
    while the request is still HTTP when transport or timeout failures occur.
    Preserve a non-`101` upstream rejection status and safe end-to-end headers,
    but replace its potentially incomplete body and strip hop-by-hop,
    WebSocket-handshake, and representation metadata. Forward end-to-end
    request headers, generate trusted `X-Forwarded-*` metadata, reflect only
...
14. **Complete (2026-08-03):** centralize fixed and `Connection`-nominated
    hop-by-hop filtering for buffered HTTP, SSE, gRPC, and WebSocket proxy
    boundaries. Preserve gRPC `TE: trailers`, multiple end-to-end response
    fields, and trusted forwarding metadata. Real HTTP/SSE entrypoint and
    WebSocket backend captures cover both request and response isolation.
15. **Complete (2026-08-03):** replace the body-buffered gRPC adapter with
    full-duplex HTTP/2 frame forwarding over h2c. Preserve upstream
    trailers, apply the service's first-response, idle-stream, and absolute
    total bounds, retain backend/access-log/TTFT accounting through the body
    lifetime, decide mirror sampling before optional collection and against the
    final resolved service, and buffer only a selected call that requires one
    replayable request. A real h2c fixture proves request/response overlap with
    a disabled mirror, trailer relay, and selected mirrored-body equivalence.
16. **Complete (2026-08-03):** align the native gRPC request boundary with HTTP
    and WebSocket forwarding metadata. Remove raw forwarding fields from the
    copied header set before rebuilding normalized address/host chains and
    authoritative scheme/port values; normalize `TE` to the only
    HTTP/2-compatible `trailers` token; and filter hop-by-hop fields from
    request and response trailer frames. The real h2c full-duplex fixture
    captures the regenerated metadata, normalized `TE`, and an end-to-end
    request trailer upstream.
17. **Complete (2026-08-03):** remove the unreachable body-collecting gRPC
    compatibility adapter, its process-wide timeout field, the duplicate status
    model, and the module-wide dead-code exemption. One bounded response frame
    relay now owns every runtime gRPC response, while a selected mirror buffers
    only the replayable request.
18. **Complete (2026-08-03):** replace the marker-only `compress` middleware
    with real Brotli, gzip, and HTTP-compatible zlib-wrapped deflate transforms
    for ordinary and Gateway-native buffered responses. Negotiate exact
    `Accept-Encoding` tokens and quality weights, offload compression from the
    async executor, rebuild length, coding, variance, validator, range, and
    digest metadata, and skip small, encoded, ranged, `no-transform`, binary,
    SSE, and gRPC responses. Unit coverage and a real Gateway/backend fixture
    prove compressed and explicitly excluded identity responses. Ordinary HTTP
    response streaming was delivered by the following item.
19. **Complete (2026-08-03):** replace both ordinary HTTP response-collecting
    paths with one full-duplex Hyper relay. Stream DATA and safe trailer frames
    with downstream backpressure and per-service response-header, idle-body,
    and absolute total bounds; retain backend, admission, request telemetry,
    TTFT, access-log, and durable usage guards through body completion or
    disconnect. Preserve response-start fallback semantics so a truncated body
    terminates the existing status instead of replaying another target. Limit
    compression look-ahead to 8 MiB, replay an overflowing prefix before the
    untouched remainder, and drain mirror responses without collecting them.
    Real Gateway/backend fixtures prove first-chunk delivery before upstream
    completion, idle termination, and overlapping request/response progress.
20. **Complete (2026-08-03):** replace the ordinary HTTP-only upstream
    connector with one Rustls-backed HTTP/HTTPS pool. Verify server certificates
    and hostnames against built-in WebPKI roots, negotiate HTTP/1.1 or HTTP/2
    through ALPN, and preserve the same full-duplex relay, timeout, fallback,
