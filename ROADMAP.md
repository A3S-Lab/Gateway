...
- accept a complete logical-scope snapshot with stable Gateway identity,
  revision, digest, and bounded validity metadata;
- validate that managed snapshots contain no local mutation sources or control
  loops;
- apply the snapshot transactionally without disrupting unchanged listeners;
- report exact applied or rejected status without claiming Cloud operation
  success;
- keep the prior snapshot on validation, bind, certificate, or reload failure;
  and
- expose readiness only for the exact applied revision requested by Cloud.
