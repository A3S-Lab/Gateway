...
| Bounded untrusted snapshot validation | `src/managed_snapshot.rs`, parser regressions, and real Node API tests reject malformed, stale, conflicting, expired, or digest-tampered input without replacing live state. After `expires_at`, the live HTTP listener fails closed with `503` / `Managed snapshot expired` and does not forward upstream (`entrypoint::tests::managed_snapshot_expiry_rejects_new_requests_on_the_listener_with_503`). | Cloud rejects incomplete, ambiguous, mixed-revision, mixed-port, and unknown management-protocol projections before activation. |
