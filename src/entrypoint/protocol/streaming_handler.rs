...
pub async fn handle_sse_dispatch(ctx: ProtocolContext) -> Response<ResponseBody> {
    ...
    match crate::proxy::streaming::forward_streaming(&backend, &req_parts.method, &req_parts.uri, &req_parts.headers, body_bytes, 300).await {
        Ok(stream_resp) => {
            let status_code = stream_resp.status.as_u16();
            let _ = access_tracker.build_entry(
                remote_addr.ip().to_string(), method_str, path, host, status_code, 0,
            ...
            let mapped = stream_resp.body_stream.map(|result| result.map(Frame::data).map_err(std::io::Error::other));
            ...
            for (key, value) in stream_resp.headers.iter() { resp_builder = resp_builder.header(key, value); }
            ...
            if let (Some(new_id), Some(sticky_mgr)) = (&sticky_new_session, state.sticky_managers.get(&route.service_name)) {
