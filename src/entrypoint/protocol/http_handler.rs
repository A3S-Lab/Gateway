...
pub async fn handle_http_dispatch(ctx: ProtocolContext) -> Response<ResponseBody> {
    ...
    match state.http_proxy.forward(&backend, &req_parts.method, &req_parts.uri, &req_parts.headers, body_bytes).await {
        Ok(proxy_resp) => {
            ...
            let _ = access_tracker.build_entry(
                ...
                proxy_resp.body.len() as u64, Some(backend.url.clone()),
            ...
            for (key, value) in proxy_resp.headers.iter() { resp_builder = resp_builder.header(key, value); }
            ...
            builder.body(crate::entrypoint::protocol::full_body(proxy_resp.body)).unwrap()
        }
        Err(e) => {
