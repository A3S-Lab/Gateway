...
| Scaling | Local scale-to-zero buffering and autoscaling from observed healthy backends, active operations, and queue depth; the controller obtains the current replica count from the selected executor before deciding and reconciles again after an ambiguous failure, with bounded executor queries and mutations | Experimental, standalone only; Box/Kubernetes end-to-end executor conformance, versioned idempotent operations, and real process/executor recovery evidence remain open |
