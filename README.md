...
**A3S Gateway is the AI Native Traffic Layer** for HTTP, SSE, WebSocket, gRPC,
TCP, and UDP model traffic. It evaluates each request against one validated
runtime snapshot and forwards it to an allowed healthy backend. Configuration
is compiled before cutover. A failed startup or reload leaves the prior proven
...
The published baseline is generated by Criterion in GitHub Actions from commit
...
These are internal operation costs, not end-to-end requests per second. They
exclude sockets, TLS, the upstream, response bodies, and clients. Inspect the
[workflow run](https://github.com/A3S-Lab/Gateway/actions/runs/30876947829),
[machine-readable results](website/assets/performance-data.json), and
