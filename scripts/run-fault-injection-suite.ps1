...
# Platform-gated probes; list names in both runners, execute only where valid.
if (-not ($IsWindows -or $env:OS -match "Windows")) {
  $Filters += "spool_storage_is_private_and_insecure_permissions_fail_closed"
  $Filters += "validate_activation_fails_closed_when_usage_spool_directory_is_not_writable"
  $Filters += "probe_activation_fails_closed_when_usage_spool_directory_is_not_writable"
  $Filters += "linked_managed_service_state_fails_gateway_construct_closed"
  $Filters += "broadly_readable_managed_service_state_fails_gateway_construct_closed"
  $Filters += "validate_rejects_docker_unix_socket_host_when_path_missing"
} else {
  $Filters += "default_docker_block_fails_validate_on_windows"
  $Filters += "validate_rejects_docker_unix_socket_host_on_non_unix"
}

$FeaturesArg = "kube,redis,wire"

Write-Host "fault-injection: running curated suite ($($Filters.Count) cases; features=$FeaturesArg)"
$failed = $false
foreach ($filter in $Filters) {
  Write-Host "fault-injection: case $filter"
  $log = Join-Path ([System.IO.Path]::GetTempPath()) ("a3s-fault-" + [guid]::NewGuid().ToString() + ".log")
  try {
    cmd /c "cargo test --lib --features $FeaturesArg $filter -- --nocapture > `"$log`" 2>&1"
    $status = $LASTEXITCODE
    Get-Content -Raw $log | Write-Host
    $text = Get-Content -Raw $log
    if ($status -ne 0) {
      Write-Host "fault-injection: FAIL $filter (cargo exit $status)" -ForegroundColor Red
      $failed = $true
    } elseif ($text -match "running 0 tests") {
      Write-Host "fault-injection: FAIL $filter (no tests matched filter)" -ForegroundColor Red
      $failed = $true
    }
  } finally {
...
if ($failed) {
  Write-Host "fault-injection: suite failed" -ForegroundColor Red
  exit 1
}
Write-Host "fault-injection: OK"
exit 0
